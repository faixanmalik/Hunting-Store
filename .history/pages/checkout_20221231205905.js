        ...
        {Object.keys(cart).map((item)=>{ return <div key={item} className="flex flex-col rounded-lg bg-white sm:flex-row">
          ...
          <div className="flex w-full flex-col px-4 py-4">
            ...
            <div className='flex'>
              ...
              <p className="flex text-md mx-auto">Qty: <AiFillMinusCircle onClick={()=>{removeFromCart(item,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> <span className='mx-2'>{cart[item].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(item,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/></p>
            </div>
          </div>
        </div>
        })} 
